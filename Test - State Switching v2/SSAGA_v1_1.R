SSAGA11 = function(fitness_func, max_iterations, max_runtime, max_stagnation, input_width) {
  ...
  while(iterations < max_iterations && (CurrentTime - CodeEnterTime) < max_runtime && populationStagnationIndex < max_stagnation && CurrentFitness > FitnessTarget) {
    ...
    CurrentFitness = fitness_func(OldPopulation[1,])
    ...
    if((populationStagnationIndex %% StateSwitchDelay == 0) && (populationStagnationIndex != 0)) {
      ...
      #Draw a new state, and check to make sure it's different than the current state
      NewStateBit = sample(1:4, 1)
      if (CurrentStateBit == NewStateBit) {
        NewStateBit = sample(1:4, 1)
      }
      CurrentStateBit = NewStateBit
      
      CurrentState = StatesList[,CurrentStateBit]
      
      #Change the time until the state switches again
      StateSwitchDelay = 2 * StateSwitchDelay
      
      #print(iterations)
      #print(CurrentStateBit)
    }
